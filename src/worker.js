...
async function handleApi(request, env, url) {
  ...
  if (request.method === 'GET' && url.pathname === '/api/health') {
    ...
    return json({ ok: true, database: result?.ok === 1, environment: env.ENVIRONMENT || 'local' }, 200, request, env);
  }
...
async function serveAsset(request, env, url) {
  ...
  const rewritePath = rewrites.get(url.pathname);
  if (!rewritePath) return env.ASSETS.fetch(request);
