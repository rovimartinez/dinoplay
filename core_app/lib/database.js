...
const Database = {
  ...
  deleteResult(idOrPin) {
    if (!idOrPin) return false;
    const initialLen = (db.results || []).length;
    db.results = (db.results || []).filter(r => r.id !== idOrPin && r.pin !== idOrPin);
    db.matches = (db.matches || []).filter(m => m.id !== idOrPin && m.pin !== idOrPin);
    if (db.live_scores && db.live_scores[idOrPin]) {
      delete db.live_scores[idOrPin];
    }
    const deleted = (db.results || []).length < initialLen;
    flushToDisk();
    return deleted;
  },
