...
(function () {
    ...
    Runner.prototype = {
        ...
        loadImages: function () {
            ...
            spriteImg.src = IS_HIDPI ? SPRITE_2X_SRC : SPRITE_1X_SRC;

            var processAndInit = function() {
                try {
                    var offCanvas = document.createElement('canvas');
                    var width = spriteImg.naturalWidth || spriteImg.width || (IS_HIDPI ? 2441 : 1233);
                    var height = spriteImg.naturalHeight || spriteImg.height || (IS_HIDPI ? 130 : 68);
                    ...
                    function colorRegion(x0, x1, y0, y1, rT, gT, bT) {
                        var startX = isHD ? x0 * 2 : x0;
                        var endX = isHD ? x1 * 2 : x1;
                        var startY = isHD ? y0 * 2 : y0;
                        var endY = isHD ? y1 * 2 : y1;
                    ...
                    // Dino custom color (en HDPI el sprite del Trex comienza en 1678, en LDPI en 848)
                    var dinoRgb = self.customDinoColor || { r: 46, g: 125, b: 50 };
                    var trexStartX = isHD ? 1678 : 848;
                    var trexEndX = isHD ? 2441 : 1233;
                    var trexStartY = 0;
                    var trexEndY = isHD ? 100 : 50;
                    for (var y = trexStartY; y < trexEndY && y < height; y++) {
                        for (var x = trexStartX; x < trexEndX && x < width; x++) {
                            var idx = (y * width + x) * 4;
                            var a = data[idx + 3];
                            var r = data[idx];
                            var g = data[idx + 1];
                            ...
                            if (a > 80 && (r < 180 || g < 180 || b < 180)) {
                                data[idx] = dinoRgb.r;
                                data[idx + 1] = dinoRgb.g;
                                data[idx + 2] = dinoRgb.b;
                            }
                        }
                    }
                    ...
                    colorRegion(134, 228, 0, 45, 229, 57, 53);    // Pterodactyl (Red)
                    colorRegion(86, 134, 0, 20, 66, 165, 245);    // Clouds (Blue)
                    colorRegion(0, 1200, 52, 70, 141, 110, 99);   // Horizon (Earth)
                    colorRegion(645, 655, 0, 15, 255, 214, 0);    // Stars (Gold)
                    colorRegion(484, 645, 0, 45, 255, 238, 88);   // Moon (Gold)
                    colorRegion(0, 40, 0, 35, 0, 172, 193);       // Restart (Cyan)
                    colorRegion(655, 835, 12, 24, 211, 47, 47);   // Game Over (Red)

                    ctx.putImageData(imgData, 0, 0);
                    Runner.imageSprite = offCanvas;
                } catch(e) {
                    Runner.imageSprite = spriteImg;
                }
                self.init();
                if (self.onEngineReady) self.onEngineReady();
            };

            if (spriteImg.complete && spriteImg.naturalWidth > 0) {
                processAndInit();
            } else {
                spriteImg.onload = processAndInit;
                setTimeout(function() {
                    if (!Runner.imageSprite) processAndInit();
                }, 50);
            }
        },
         ...
         */
        init: function () {
            var icon = document.querySelector('.' + Runner.classes.ICON); if (icon) icon.style.visibility = 'hidden';
        ...
        adjustDimensions: function () {
            ...
            var boxStyles = this.outerContainerEl ? window.getComputedStyle(this.outerContainerEl) : null;
            var padding = boxStyles ? Number(boxStyles.paddingLeft.substr(0, boxStyles.paddingLeft.length - 2)) : 0;

            var rawWidth = (this.outerContainerEl && this.outerContainerEl.offsetWidth) ? (this.outerContainerEl.offsetWidth - padding * 2) : DEFAULT_WIDTH;
            if (rawWidth <= 0) rawWidth = DEFAULT_WIDTH;

            this.dimensions.WIDTH = Math.min(DEFAULT_WIDTH, rawWidth);
            this.dimensions.HEIGHT = (Runner.defaultDimensions && Runner.defaultDimensions.HEIGHT) ? Runner.defaultDimensions.HEIGHT : DEFAULT_HEIGHT;
        ...
        update: function () {
            ...
            var rawDelta = now - (this.time || now);
            this.time = now;
            var deltaTime = Math.min(rawDelta, 100);

            if (this.playing) {
                if (this.onStateUpdate) {
                    var currentAction = 'running';
                    if (this.tRex.jumping) currentAction = 'jumping';
                    else if (this.tRex.ducking) currentAction = 'ducking';
                    if (this.crashed) currentAction = 'crashed';

                    var currentObstacles = [];
                    if (this.horizon && this.horizon.obstacles) {
                        for (var i = 0; i < this.horizon.obstacles.length; i++) {
                            var ob = this.horizon.obstacles[i];
                            currentObstacles.push({
                                x: Math.round(ob.xPos),
                                y: Math.round(ob.yPos),
                                type: ob.typeConfig.type,
                                size: ob.size,
                                width: ob.typeConfig.width * ob.size,
                                height: ob.typeConfig.height
                            });
                        }
                    }
                ...
                // Check for collisions across all visible obstacles
                var collision = false;
                var collidedObstacleIndex = -1;
                if (hasObstacles && this.horizon.obstacles.length > 0 && !this.invulnerable) {
                    for (var obsIdx = 0; obsIdx < this.horizon.obstacles.length; obsIdx++) {
                        var obs = this.horizon.obstacles[obsIdx];
                        // Solo chequear obstáculos que estén en el rango del dino
                        if (obs.xPos > 150) break; // ordenados por xPos
                        if (obs.xPos + obs.width < 0) continue;
                        if (checkForCollision(obs, this.tRex)) {
                            collision = true;
                            collidedObstacleIndex = obsIdx;
                            break;
                        }
                    }
                }
         ...
         */
        setArcadeMode() {
            // Keep game size stable without jumping
        },
        ...
        playSound: function (type) {
            try {
                ...
                if (type === this.soundFx.BUTTON_PRESS || type === 'BUTTON_PRESS' || type === 'offline-sound-press') {
                    ...
                    osc.start(now);
                    osc.stop(now + 0.1);
                } else if (type === this.soundFx.HIT || type === 'HIT' || type === 'offline-sound-hit') {
                    var osc = ctx.createOscillator();
                    var gain = ctx.createGain();
                    osc.type = 'sawtooth';
                    ...
                    osc.start(now);
                    osc.stop(now + 0.25);
                } else if (type === this.soundFx.SCORE || type === 'SCORE' || type === 'offline-sound-reached') {
                    ...
                    osc2.type = 'sine';
                    osc2.frequency.setValueAtTime(900, now + 0.09);
    ...
    function checkForCollision(obstacle, tRex, opt_canvasCtx) {
        ...
        if (boxCompare(shrinkCollisionBox(tRexBox, 2, 2),
            shrinkCollisionBox(obstacleBox, 2, 1))) {
            ...
            for (var t = 0; t < tRexCollisionBoxes.length; t++) {
                for (var i = 0; i < collisionBoxes.length; i++) {
                    ...
                    var crashed = boxCompare(
                        shrinkCollisionBox(adjTrexBox, 2, 2),
                        shrinkCollisionBox(adjObstacleBox, 2, 1));
    ...
    /**
     * Slightly reduce collision boxes so transparent sprite padding does not feel
     * like a hit before the player visually touches an obstacle.
     */
    function shrinkCollisionBox(box, xPadding, yPadding) {
        var width = Math.max(0, box.width - xPadding * 2);
        var height = Math.max(0, box.height - yPadding * 2);
        return new CollisionBox(
            box.x + xPadding,
            box.y + yPadding,
            width,
...
// Controlled by createDinoGame
    ...
    window.createDinoGame = function(containerSelector, options) {
        ...
        runner.onStateUpdate = options.onStateUpdate || null;
        runner.onCrash = options.onCrash || null;
        runner.onEngineReady = options.onEngineReady || null;

        if (runner.canvas && options.onEngineReady) {
            setTimeout(options.onEngineReady, 10);
        }
